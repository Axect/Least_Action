...
for i in range(1, species+1):
    ...
    dcc = np.array(dh[f'dcc_{i}'][:2**i-1])
    #bf = np.insert(bf, 0, start)
    #bf = np.append(bf, end)
    dc = np.insert(dc, 0, start)
    dc = np.append(dc, end)
    dcc = np.insert(dcc, 0, start)
    dcc = np.append(dcc, end)
    #bfs.append(bf)
    dcs.append(dc)
    dccs.append(dcc)
styles = ['-', '--', '-', '-.', ':']
...
with plt.style.context(["science", "nature"]):
    ...
    ax.plot(t_true, q_true, 'k', label=r'$q_\text{true}~(\omega=1, T=\frac{2}{3}\pi)$', alpha=0.3)
    for i in range(1, species+1):
        ax.plot(t_hats[i-1], dccs[i-1], styles[i-1], label=rf'$\hat{{q}}_\text{{dcc}} ~(N=2^{i} - 1)$', alpha=0.7)
    ax.legend()
    fig.savefig('harmonic_dcc.png', dpi=600, bbox_inches='tight')
